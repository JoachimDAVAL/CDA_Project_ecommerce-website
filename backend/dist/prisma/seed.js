...
async function main() {
    ...
    const model1 = await prisma.model3D.create({
        data: {
            ...
            description: 'Une épée optimisée pour les jeux mobiles. Textures 4K incluses. Parfaite pour des jeux d\'action-aventure avec un style cartoon.',
            price: 15.99,
            status: 'ONLINE',
            viewCount: 42,
    ...
    const model3 = await prisma.model3D.create({
        data: {
            ...
            description: 'Un pack complet de 50+ plantes et arbres low poly. Idéal pour créer des forêts stylisées. Textures optimisées.',
            price: 29.99,
            status: 'ONLINE',
            viewCount: 128,
            artistId: artistUser.id,
    ...
    const model4 = await prisma.model3D.create({
        data: {
            ...
            description: 'Personnage stylisé avec rig complet et animations de base (idle, walk, run, attack). Compatible avec Unity et Unreal Engine.',
            price: 79.99,
            status: 'ONLINE',
            viewCount: 256,
    ...
    const model5 = await prisma.model3D.create({
        data: {
            ...
            description: 'Kit complet de pièces modulaires pour construire votre propre château. Plus de 100 éléments inclus (murs, tours, portes, décorations).',
            price: 49.99,
            status: 'ONLINE',
            viewCount: 89,
            artistId: artistUser2.id,
    ...
    const review1 = await prisma.review.create({
        data: {
            rating: 5,
            comment: 'Excellente qualité ! Parfait pour mon jeu mobile. Les textures sont magnifiques et le modèle est très bien optimisé.',
